...
function install(callback) {
  ...
  const folderName = getMeName(opts.url);
...
function getMeName(url) {
  const fileNameRegex = /\/([^/]+\.tar\.gz)$/;
  const match = url.match(fileNameRegex);

  if (match) {
    const fileName = match[1]; // The matched part in the first capturing group
    const cleanFileName = fileName.replace(/\.tar\.gz$/, ""); // Remove the .tar.gz extension
    console.log(cleanFileName);
    return cleanFileName;
  } else {
    console.log("File name not found in the URL.");
  }
}
