...
func (h *HttpClient) RunScen(ctx context.Context, conf model.PayloadResponder) {
	finished := make(chan error)
	// timeout := time.After(time.Duration(conf.Conf.Time) * time.Second)
	var err error
	go func() {
		<-time.After(time.Duration(conf.Conf.Time) * time.Second)
		fmt.Println("timeout END HERE")
	}()
	go func() {
		select {
		case err = <-finished:
...
func (h *HttpClient) Manager(ctx context.Context, conf model.PayloadResponder, done chan<- error) {
	headers := h.GetRequestHeades(conf.Conf.Headers)
	numOfClient := conf.Conf.Clients
	var wg sync.WaitGroup
	wg.Add(numOfClient)
	body, _ := json.Marshal(conf.Conf.PostData)
	conf.Conf.Requests = int64((1 << 63) - 1)
			...
			go func() {
				for i := 0; i < 10; i++ {
					for {
						...
						select {
						case <-ctx.Done():
							fmt.Println("TIME OUT Hit In Request Lopp")
							return
						default:
							if conf.Conf.Method == "GET" {
								h.Get(ctx, conf.Conf.URL, headers)
							} else {
								h.Request(conf.Conf.Method, conf.Conf.URL, body, headers)
							}
							reqCount++
						}
					}
